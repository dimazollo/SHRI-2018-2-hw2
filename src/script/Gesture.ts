...
export default class Gesture {
  ...
  translateParams: Point;
  _scaleParam: number;
  _brightnessParam: number;
  prevPinchDistance: number | null;
  prevRotateAngle: number | null;

  constructor() {
    this.pointers = [];
    this.translateParams = new Point(0, 0);
  ...
  handlePointerMove(event: PointerEvent) {
    ...
    if (currentPointer == null) {
      ...
      if (this.pointers.length === 1) {
        const newPoint = new Point(event.x, event.y);
  ...
  releasePointer ({ pointerId } : PointerEvent) {
  ...
  getPointer({ pointerId } : PointerEvent) {
  ...
  handlePinch(currentPointer : Pointer, secondPointer: Pointer) {
  ...
  handleRotate(currentPointer: Pointer, secondPointer: Pointer) {
